...
// ✅ Assignment submissions (🔧 fixed collection path)
...
// ✅ Classroom questions (🔧 fixed path to match frontend)
export async function postStudentQuestion(classId, studentUid, question) {
  return await addDoc(collection(db, "classes", classId, "questions"), {
    studentUid,
    question,
    createdAt: serverTimestamp()
  });
...
export async function answerStudentQuestion(classId, questionId, answer) {
  const ref = doc(db, "classes", classId, "questions", questionId);
